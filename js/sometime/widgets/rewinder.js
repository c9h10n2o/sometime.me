...
//			jPlayer container
	...
	exports.init = function(_config) {	
		...
		Sound = {
			_file: {
				'roll': '/images/btn-rewinder-snd-roll.mp2'
			,	'release': '/images/btn-rewinder-snd-release.mp2'
			}
		,	init: function() {
				if (_.IE < 7) return;
...
//		  	hold time > 200
	...
	function redraw() {
		...
		uiBtn.removeClass('hidden').addClass('invisible');
		bdW = $('#bd').width();
		uiBtn.removeClass('invisible').addClass('hidden');
		
//		r = (_.$BD.width() - bdW) / 2 * .7 - uiBtn.width();
		minH = config.minScrollScreens * _.$W.height();

//		enabled = r > 20 &&  _.PAGESCROLL.scrollTop > minH;
		enabled = _.PAGESCROLL.scrollTop > minH;

//		enabled
//		? uiBtn.css({ 'right': r }).fadeIn(_.IE < 7 ? 0 : 800)
//		: uiBtn.fadeOut(_.IE < 7 ? 0 : 800);
		enabled
		? uiBtn.fadeIn(_.IE < 7 ? 0 : 800)
		: uiBtn.fadeOut(_.IE < 7 ? 0 : 800);
	}
