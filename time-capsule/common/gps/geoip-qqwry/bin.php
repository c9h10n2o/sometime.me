...
class IpLocationSeekerBinary {
	 ...
	 */
	public function saveAsSqlite($filepath, $version=3, $use_china_province_name=true, $echo=false){
		...
		for($i=0; $i<$this->index_count; $i++){
			...
			if($use_china_province_name){
				$province = self::real_province_name($country);
				if($province){
					$area = $country;
					$country = $province;
				}
			...
			$area = str_replace("\n", "", $area);
 			...
 			if($echo) echo long2ip($ip_int), "\t", $country, "\t", $area, "\n";
			$area = str_replace("'", "", $area);
	...
	private static function real_province_name($country_in_qqwry){
		...
		static $special_maps = array("东北农业大学"=>"黑龙江", "东北大学"=>"辽宁", 
				...
				"黄河科技大学"=>"河南");
		if(isset($special_maps[$country_in_qqwry])) $province_name = $special_maps[$country_in_qqwry];
		else{
			static $province_names = NULL;
			if(!$province_names)
				$province_names = explode(",", "北京,上海,天津,重庆,安徽,福建,甘肃,广东,广西,贵州,海南,河北,河南,黑龙江,湖北,湖南,吉林,江苏,江西,辽宁,内蒙古,宁夏,青海,山东,山西,陕西,四川,西藏,新疆,云南,浙江,香港,澳门,台湾,中国,美国");
			foreach($province_names as $name){
				if(strpos($country_in_qqwry, $name) !== false
				&& $country_in_qqwry != $name)
					$province_name = $name;
			}
		}
		if(!$province_name){
			if(strpos($country_in_qqwry, "北方工业大学")!==false) $province_name = "北京";
			if(strpos($country_in_qqwry, "华南农业大学")!==false) $province_name = "广东";
			if(strpos($country_in_qqwry, "华南理工大学")!==false) $province_name = "广东";
			if(strpos($country_in_qqwry, "厦门大学")!==false) $province_name = "福建";
			if(strpos($country_in_qqwry, "同济大学")!==false) $province_name = "上海";
			if(strpos($country_in_qqwry, "大连理工大学")!==false) $province_name = "辽宁";
			if(strpos($country_in_qqwry, "成都")!==false) $province_name = "四川";
			if(strpos($country_in_qqwry, "武汉")!==false) $province_name = "湖北";
			if(strpos($country_in_qqwry, "西安")!==false) $province_name = "陕西";
			if(strpos($country_in_qqwry, "西华大学")!==false) $province_name = "四川";
			if(strpos($country_in_qqwry, "长春")!==false) $province_name = "吉林";
			if(strpos($country_in_qqwry, "长沙")!==false) $province_name = "湖南";
			if(strpos($country_in_qqwry, "首都")!==false) $province_name = "北京";
			if(strpos($country_in_qqwry, "青岛")!==false) $province_name = "山东";
			if(strpos($country_in_qqwry, "齐齐哈尔")!==false) $province_name = "黑龙江";
			if(strpos($country_in_qqwry, "暨南大学")!==false) $province_name = "广东";
		}
	...
	private function get_country_and_area($ip_record_pos){
		...
		return preg_replace("/\s*CZ88\.NET\s*/i", "", array($country, $area));
	}
